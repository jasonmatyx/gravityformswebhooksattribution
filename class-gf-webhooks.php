...
class GF_Webhooks extends GFFeedAddOn {
	 ...
	 */
	protected $_slug = 'gravityformswebhooksattribution';
	 ...
	 */
	protected $_path = 'gravityformswebhooksattribution/webhooks.php';
	...
	public function scripts() {
		...
		$scripts = array(
			array(
				...
				'strings' => array(
					'ajax_url_label'     => esc_html__( 'Admin Ajax URL', 'gravityformswebhooksattribution' ),
					'rest_api_url_label' => esc_html__( 'REST API URL', 'gravityformswebhooksattribution' ),
				),
			),
		);
	...
	public function init() {
		...
		$this->add_delayed_payment_support(
			array(
				'option_label' => esc_html__( 'Send webhook only when payment is received.', 'gravityformswebhooksattribution' ),
			)
		);
	}
	...
	public function feed_settings_fields() {
		...
		return array(
			array(
				'fields' => array(
					array(
						'label'    => esc_html__( 'Name', 'gravityformswebhooksattribution' ),
						'name'     => 'feedName',
						'type'     => 'text',
						'class'    => 'medium',
						'required' => true,
						'tooltip'  => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Name', 'gravityformswebhooksattribution' ),
							esc_html__( 'Enter a feed name to uniquely identify this setup.', 'gravityformswebhooksattribution' )
						),
					),
				),
			),
			array(
				'fields' => array(
					array(
						'label'               => esc_html__( 'Request URL', 'gravityformswebhooksattribution' ),
						'name'                => 'requestURL',
						'type'                => 'text',
						...
						'tooltip'             => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Request URL', 'gravityformswebhooksattribution' ),
							esc_html__( 'Enter the URL to be used in the webhook request.', 'gravityformswebhooksattribution' )
						),
						'validation_callback' => array( $this, 'validate_request_url' ),
					),
					array(
						'label'         => esc_html__( 'Request Method', 'gravityformswebhooksattribution' ),
						'name'          => 'requestMethod',
						'type'          => 'select',
						'default_value' => 'POST',
						'required'      => true,
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Request Method', 'gravityformswebhooksattribution' ),
							esc_html__( 'Select the HTTP method used for the webhook request.', 'gravityformswebhooksattribution' )
						),
						'choices'       => array(
							array(
								'label' => 'GET',
					...
					array(
						'label'         => esc_html__( 'Request Format', 'gravityformswebhooksattribution' ),
						'name'          => 'requestFormat',
						'type'          => 'select',
						'default_value' => 'json',
						'required'      => true,
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Request Format', 'gravityformswebhooksattribution' ),
							esc_html__( 'Select the format for the webhook request.', 'gravityformswebhooksattribution' )
						),
						'choices'       => array(
							array(
								'label' => esc_html__( 'JSON', 'gravityformswebhooksattribution' ),
								'value' => 'json',
							),
							array(
								'label' => esc_html__( 'FORM', 'gravityformswebhooksattribution' ),
								'value' => 'form',
							),
						),
					),
			...
			array(
				'fields' => array(
					array(
						'label'       => esc_html__( 'Request Headers', 'gravityformswebhooksattribution' ),
						'name'        => 'requestHeaders',
						'type'        => 'generic_map',
						'required'    => false,
						'tooltip'     => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Request Headers', 'gravityformswebhooksattribution' ),
							esc_html__( 'Setup the HTTP headers to be sent with the webhook request.', 'gravityformswebhooksattribution' )
						),
						'key_field'   => array(
							'choices'      => $this->get_header_choices(),
							'custom_value' => true,
							'title'        => esc_html__( 'Name', 'gravityformswebhooksattribution' ),
						),
						'value_field' => array(
							'choices'      => 'form_fields',
							'custom_value' => true,
			...
			array(
				'fields' => array(
					array(
						'label'         => esc_html__( 'Request Body', 'gravityformswebhooksattribution' ),
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Request Body', 'gravityformswebhooksattribution' ),
							esc_html__( 'Select if all fields or select fields should be sent with the webhook request.', 'gravityformswebhooksattribution' )
						),
						'choices'       => array(
							array(
								'label' => esc_html__( 'All Fields', 'gravityformswebhooksattribution' ),
								'value' => 'all_fields',
							),
							array(
								'label' => esc_html__( 'Select Fields', 'gravityformswebhooksattribution' ),
								'value' => 'select_fields',
							),
						),
					),
					array(
						'label'       => esc_html__( 'Field Values', 'gravityformswebhooksattribution' ),
						'name'        => 'fieldValues',
						'type'        => 'generic_map',
						'required'    => true,
						...
						'tooltip'     => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Field Values', 'gravityformswebhooksattribution' ),
							esc_html__( 'Setup the fields to be sent in the webhook request.', 'gravityformswebhooksattribution' )
						),
						'value_field' => array(
							'choices'      => 'form_fields',
							'custom_value' => true,
			...
			array(
				'fields' => array(
					array(
						'name'           => 'feedCondition',
						'type'           => 'feed_condition',
						'label'          => esc_html__( 'Webhook Condition', 'gravityformswebhooksattribution' ),
						'checkbox_label' => esc_html__( 'Enable Condition', 'gravityformswebhooksattribution' ),
						'instructions'   => esc_html__( 'Execute Webhook if', 'gravityformswebhooksattribution' ),
						'tooltip'        => sprintf(
							'<h6>%s</h6>%s',
	...
	public function feed_list_columns() {
		...
		return array(
			'feedName'   => esc_html__( 'Name', 'gravityformswebhooksattribution' ),
			'requestURL' => esc_html__( 'Request URL', 'gravityformswebhooksattribution' ),
		);
	...
	public function get_header_choices() {
		...
		return array(
			array(
				'label' => esc_html__( 'Select a Name', 'gravityformswebhooksattribution' ),
				'value' => '',
			),
			array(
				'label' => 'Accept',
	...
	public function process_feed( $feed, $entry, $form ) {
		...
		if ( rgblank( $request_url ) ) {
			$this->add_feed_error( esc_html__( 'Webhook was not processed because request URL was empty.', 'gravityformswebhooksattribution' ), $feed, $entry, $form );
		...
		if ( is_wp_error( $response ) ) {
			...
			$this->add_feed_error( sprintf( esc_html__( 'Webhook was not successfully executed. %1$s (%2$d)', 'gravityformswebhooksattribution' ), $response->get_error_message(), $response->get_error_code() ), $feed, $entry, $form );
			$return_response = true;
		} else {
			$code = wp_remote_retrieve_response_code( $response );
			/* translators: The response code */
			$this->add_note( rgar( $entry, 'id' ), sprintf( esc_html__( 'Webhook sent. Response code: %d.', 'gravityformswebhooksattribution' ), $code ), 'success' );
	...
	private function get_validation_error_message( $field, $request_url, $sanitized_field_setting ) {
		...
		$message = $this->is_url_without_scheme( $request_url )
			? esc_html__( 'The URL you have entered is malformed.', 'gravityformswebhooksattribution' )
			: esc_html__( 'The URL you have entered is not valid. For security reasons, some characters are not allowed.', 'gravityformswebhooksattribution' );
		...
		return $message
			. sprintf(
				...
				esc_html__( 'Fix it', 'gravityformswebhooksattribution' )
			);
	}
...
/*** GRAVITY FORMS WEBHOOK LABEL FIX BY JASON ***/
...
/*** END GRAVITY FORMS WEBHOOK LABEL FIX BY JASON ***/